...
_**I've already uploaded the 128x128 and 224x224 models from the `checkpoints` folder and the callable URLs are here:**_
```
...
- `resnet18_prepost_plaince_xbd_128_seed17.pt` OR `resnet18_prepost_plaince_xbd_224_seed17.pt` — the checkpoint, (can replace with another checkpoint file, update `modal_app.py` accordingly)
