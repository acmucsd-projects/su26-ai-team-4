...
Paired PRE+POST ResNet-18 experiment for xBD building-damage classification.

PURPOSE
-------

This experiment changes one primary variable from train_post_resnet18.py:

    POST-only baseline:
        POST image -> ResNet-18 -> classifier

    PRE+POST experiment:
        PRE image  -> shared ResNet-18 -> PRE features
        POST image -> shared ResNet-18 -> POST features
                                      -> concatenate
                                      -> classifier

All other major training choices are intentionally kept aligned with the
POST-only baseline so the effect of adding PRE imagery is easier to interpret.
...
└── base_train/
    └── ezekiel_resnet18_baseline/
        ...
        ├── train_prepost_resnet18.py    <-- THIS FILE
        │
        ├── cache/
        │   ├── cache_manifest.csv
        │   ├── pre/
        │   └── post/
        │
        └── results_prepost/             <-- CREATED BY THIS SCRIPT
...
From the baseline directory:
    ...
    python train_prepost_resnet18.py
...
DEFAULT_RESULTS_DIR = BASELINE_DIR / "results_prepost"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train the paired PRE+POST xBD ResNet-18 experiment.",
...
def main():
    ...
    print("=" * 80)
    print("xBD PAIRED PRE+POST RESNET-18 EXPERIMENT")
    print("=" * 80)
    print()
    ...
    summary_lines = [
        "xBD PAIRED PRE+POST RESNET-18 EXPERIMENT",
        "=" * 80,
        "",
