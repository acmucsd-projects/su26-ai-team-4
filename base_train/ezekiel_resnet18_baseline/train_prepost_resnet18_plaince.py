...
Paired PRE+POST ResNet-18 plain-cross-entropy baseline for xBD
building-damage classification.

PURPOSE
-------

This experiment changes one primary variable from train_prepost_resnet18.py:
...
The model, data split, image size, augmentation, optimizer, learning rate,
batch size, and checkpoint metric are intentionally kept aligned with the
paired PRE+POST baseline so the effect of removing class weighting is easier
to interpret.
...
└── base_train/
    └── ezekiel_resnet18_baseline/
        ...
        ├── train_prepost_resnet18_plaince.py  <-- THIS FILE
        │
        ├── cache/
        │   ├── cache_manifest.csv
        │   ├── pre/
        │   └── post/
        │
        └── results_prepost_plaince/     <-- CREATED BY THIS SCRIPT
...
From the baseline directory:
    ...
    python train_prepost_resnet18_plaince.py
...
The PRE and POST images for each building come from the same cache row.

EXPERIMENT DESIGN
-----------------

The same ImageNet-pretrained ResNet-18 backbone is applied to PRE and POST
images. The weights are shared between the two branches.

Each image produces a 512-dimensional feature vector. The two vectors are
concatenated into a 1024-dimensional representation and passed to the same
style of classifier used by the POST-only baseline:

    Linear(1024 -> 512)
    ReLU
    Linear(512 -> 4)

Geometric augmentation is synchronized across each PRE/POST pair: the same
horizontal flip, vertical flip, and rotation are applied to both images.
This avoids introducing artificial misalignment between the paired views.

The following baseline choices are intentionally unchanged:
    ...
    learning rate:     1e-4
    weight decay:      1e-4
    loss:              plain cross entropy (no class weights)
    split:             scene-disjoint
    validation cap:    15,000 buildings
    seed:              42
    split seed:        42
...
The reported Macro F1 is a validation metric used for model selection, not an
untouched final test-set estimate.
"""
...
DEFAULT_RESULTS_DIR = BASELINE_DIR / "results_prepost_plaince"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train the paired PRE+POST xBD ResNet-18 plain-CE baseline.",
...
def load_cache_manifest(path: Path) -> pd.DataFrame:
    """
    Load the prepared cache and resolve paired PRE/POST image paths.

    pre_png and post_png are stored relative to cache_manifest.csv, so the cache directory
    ...
    # Fail early if either side of the paired cache is incomplete.
    missing_pre = [
        path
        for path in df["pre_path"]
    ...
    if missing_pre or missing_post:
        ...
        raise SystemExit(
            "PRE/POST cache is incomplete.\n\n"
            f"Missing PRE files:  {len(missing_pre):,}\n"
            f"Missing POST files: {len(missing_post):,}\n"
            f"Example:\n    {example}\n\n"
...
class PrePostDataset(Dataset):
    """Dataset for paired cached PRE- and POST-disaster building crops."""
    ...
    def __init__(
        ...
        self.pre_paths = df["pre_path"].tolist()
        self.post_paths = df["post_path"].tolist()
    ...
    def __getitem__(self, index: int):
        with Image.open(
            self.pre_paths[index]
        ) as image:
            pre_image = image.convert("RGB").copy()

        with Image.open(
            self.post_paths[index]
        ) as image:
            post_image = image.convert("RGB").copy()

        # Use the SAME geometric augmentation for both images in a pair.
        if self.training:
            use_hflip = random.random() < 0.5
            use_vflip = random.random() < 0.5
        ...
        pre_tensor = (
            pre_tensor - IMAGENET_MEAN
        ) / IMAGENET_STD

        post_tensor = (
            post_tensor - IMAGENET_MEAN
        ) / IMAGENET_STD
...
class PrePostResNet18(nn.Module):
    """
    Shared-weight ResNet-18 for paired PRE+POST classification.

    PRE and POST images pass through the same ImageNet-pretrained backbone.
    Their 512-dimensional feature vectors are concatenated before the
    four-class damage classifier.
    """
    ...
    def forward(
        ...
        pre_features = self.backbone(
            pre_images
        )

        post_features = self.backbone(
            post_images
        )
...
# Standard cross entropy is used without class weighting. This keeps the
# natural class distribution in the optimization objective.
...
def train_one_epoch(
    ...
    for pre_images, post_images, targets, _ in loader:
...
def evaluate(
    ...
    for pre_images, post_images, targets, ids in loader:
        pre_images = pre_images.to(
            device,
            non_blocking=True,
            memory_format=torch.channels_last,
...
def main():
    ...
    print("=" * 80)
    print("xBD PAIRED PRE+POST RESNET-18 PLAIN-CE BASELINE")
    print("=" * 80)
    print()
    ...
    print("Loss:            plain cross entropy (no class weights)")
    ...
    for epoch in range(
        ...
        if (
            ...
            torch.save(
                {
                    "model_name": "prepost_resnet18",
                    "epoch": epoch,
                    "model_state_dict": model.state_dict(),
                    "class_names": CLASS_NAMES,
                    "val_metrics": val_metrics,
                    "config": {
                        "input_mode": "prepost_shared_backbone_concat",
                        "loss": "cross_entropy",
    ...
    result = {
        "model": "prepost_resnet18_plaince",
        "input_mode": "prepost_shared_backbone_concat",
        "loss": "cross_entropy",
        "class_weighting": False,
    ...
    summary_lines = [
        "xBD PAIRED PRE+POST RESNET-18 PLAIN-CE BASELINE",
        "=" * 80,
        "",
